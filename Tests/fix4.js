...
const startIndex = lines.findIndex(l => l.includes('                <nav className="relative z-0 inline-flex rounded-2xl shadow-sm -space-x-px gap-2"'));
const endIndex = lines.findIndex(l => l.includes('      {/* Order Details Panel */}'));
...
const replacement = `                <nav className="relative z-0 inline-flex rounded-2xl shadow-sm -space-x-px gap-2" aria-label="Pagination">
                  <motion.button
...
fs.writeFileSync('src/pages/orders/OrdersList.tsx', newLines);
console.log("FIXED");
